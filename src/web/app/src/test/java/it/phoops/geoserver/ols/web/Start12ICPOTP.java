 ...
 *
 */
public class Start12ICPOTP {
    private static final Logger log = org.geotools.util.logging.Logging.getLogger(Start12ICPOTP.class.getName());
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            WebAppContext geoServerWAH = new WebAppContext();
            geoServerWAH.setContextPath("/geoserver");
            geoServerWAH.setWar("src/main/webapp");
            
            geoServerWAH.setTempDirectory(new File("target/work"));
            //this allows to send large SLD's from the styles form
            geoServerWAH.getServletContext().getContextHandler().setMaxFormContentSize(1024 * 1024 * 2);
            jettyServer.addHandler(geoServerWAH);

            WebAppContext solrWAH = new WebAppContext();
            solrWAH.setContextPath("/solr");
            solrWAH.setWar("../../../../solr-4.1.0/dist/solr-4.1.0.war");
            
            solrWAH.setTempDirectory(new File("/tmp"));
            //this allows to send large SLD's from the styles form
            solrWAH.getServletContext().getContextHandler().setMaxFormContentSize(1024 * 1024 * 2);
            jettyServer.addHandler(solrWAH);
