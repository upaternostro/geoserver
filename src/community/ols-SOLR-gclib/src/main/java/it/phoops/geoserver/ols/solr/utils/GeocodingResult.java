...
public class GeocodingResult {
    ...
    public GeocodingResult(OLSAddressBean olsAddressBean) {
        formattedAddress = olsAddressBean.getName()+olsAddressBean.getBuildingNumber()+olsAddressBean.getNumberExtension()+olsAddressBean.getNumberColor();
     ...
     * address components. For example, the address "111 8th Avenue, New York, NY" contains separate
     * address components for "111" (the street number, "8th Avenue" (the route), "New York" (the
     * city) and "NY" (the US state). These address components contain additional information.
     */
    ...
    /**
     * The {@code types} array indicates the type of the returned result. This array contains a set
     * of zero or more tags identifying the type of feature returned in the result. For example, a
    ...
    /**
     * {@code partialMatch} indicates that the geocoder did not return an exact match for the
     * original request, though it was able to match part of the requested address. You may wish to
     * examine the original request for misspellings and/or an incomplete address.
     *
